    ...
    <div className="w-full py-2 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          ...
          <p className="text-base text-slate-500 font-medium">Manage and track your shop products</p>
        </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm transition-shadow hover:shadow-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input required type="text" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-[#1D9E75]/20 focus:border-[#1D9E75] transition-all" placeholder="Enter product name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[13px] font-bold text-slate-600 uppercase tracking-widest mb-1.5 block">Category</label>
                  <input list="cat-list" required type="text" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1D9E75]" placeholder="Enter category..." value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} />
                  <datalist id="cat-list">{categories.map(c => <option key={c} value={c} />)}</datalist>
                </div>
                <div>
                  <label className="text-[13px] font-bold text-slate-600 uppercase tracking-widest mb-1.5 block">Unit</label>
                  <select className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1D9E75] appearance-none" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value as Unit })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="text" inputMode="decimal" placeholder="enter buy price" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#1D9E75]" value={formData.current_buy_price} onChange={e => setFormData({ ...formData, current_buy_price: e.target.value.replace(/[^0-9.]/g, "") })} />
                </div>
                <div>
                  <label className="text-[13px] font-bold text-slate-600 uppercase tracking-widest mb-1.5 block">Sell Price (₹)</label>
                  <input required type="text" inputMode="decimal" placeholder="enter sell price" className={`w-full p-2.5 bg-slate-50 border rounded-xl outline-none focus:ring-2 transition-all ${isLosingMoney ? 'border-amber-400 focus:ring-amber-100' : 'border-slate-200 focus:border-[#1D9E75] focus:ring-[#1D9E75]/10'}`} value={formData.current_sell_price} onChange={e => setFormData({ ...formData, current_sell_price: e.target.value.replace(/[^0-9.]/g, "") })} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" inputMode="numeric" placeholder="enter initial stock" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none" value={formData.current_stock} onChange={e => setFormData({ ...formData, current_stock: e.target.value.replace(/\D/g, "") })} />
                </div>
                <div>
                  <label className="text-[13px] font-bold text-slate-600 uppercase tracking-widest mb-1.5 block">Low Alert</label>
                  <input type="text" inputMode="numeric" placeholder="enter low alert threshold" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none" value={formData.low_stock_threshold} onChange={e => setFormData({ ...formData, low_stock_threshold: e.target.value.replace(/\D/g, "") })} />
                </div>
              </div>
              ...
              <button
                disabled={btnLoading}
                className="w-full py-4 bg-[#1D9E75] text-white cursor-pointer rounded-2xl font-black text-sm shadow-lg shadow-green-100 hover:bg-[#168a65] transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              {editingProductId && (
                <button
                  ...
                  className="w-full py-3 border border-slate-200 bg-white text-slate-600 cursor-pointer rounded-2xl font-semibold text-sm hover:bg-slate-50 transition-all"
                >
                  Cancel Edit
                </button>
              )}
            ...
            <table className="w-full text-left border-collapse min-w-175">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-400 text-xs uppercase font-bold tracking-widest">
                <tr>
                  ...
                  <th className="px-6 py-4">Margin Info</th>
                  <th className="px-6 py-4 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  [...Array(5)].map((_, i) => <tr key={i} className="animate-pulse h-20 bg-slate-50/50"></tr>)
                      ...
                      <tr key={p._id} className={`group transition-all duration-500 ${isNew ? 'bg-green-50/50 shadow-inner' : 'hover:bg-slate-50/50'} ${p.is_active === false ? 'opacity-70' : ''}`}>
                        <td className="px-6 py-5">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[15px] text-slate-500 font-medium">unit: {p.unit}</span>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <span className="text-[15px] font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg italic">{p.category}</span>
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex flex-col">
                            <span className="text-[17px] font-bold text-slate-900">₹{p.current_sell_price}</span>
                            <span className="text-[15px] text-slate-500">cost: ₹{p.current_buy_price}</span>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                              <span className="text-[17px] font-bold text-[#1D9E75]">₹{margin.amount}</span>
...
function StatCard({ label, value, color = "text-slate-800", sub }: any) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-transform hover:-translate-y-1 duration-300">
